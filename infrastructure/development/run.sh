...
#   ./run.sh [development|staging|production]
# parameters:
# $1 = environment where variables will be set. "development" by default
...
function set_development_environment() {
  ...
  export OSIRIS_SENSORNET_JAR_FILE_NAME=SensorNet-${OSIRIS_SENSORNET_MAJOR_VERSION}.jar
  ...
  export OSIRIS_VIRTUALSENSORNET_JAR_FILE_NAME=VirtualSensorNet-${OSIRIS_SENSORNET_MAJOR_VERSION}.jar
}
...
function build_docker_image_postgresql() {
  ...
  docker build -t $DOCKER_IMAGE_NAME_POSTGRESQL postgresql \
  --build-arg POSTGRESQL_PORT=${POSTGRESQL_PORT} \
  --build-arg POSTGRESQL_USER_NAME=${POSTGRESQL_USER_NAME} \
  --build-arg POSTGRESQL_USER_PASSWORD=${POSTGRESQL_USER_PASS} \
  --build-arg POSTGRESQL_SENSORNET_DB_NAME=${POSTGRESQL_SENSORNET_DB_NAME} \
  --build-arg POSTGRESQL_SENSORNETWEB_DB_NAME=${POSTGRESQL_SENSORNET_WEB_DB_NAME} \
  --build-arg POSTGRESQL_VIRTUALSENSORNET_DB_NAME=${POSTGRESQL_VIRTUALSENSORNET_DB_NAME} \
  --build-arg POSTGRESQL_VIRTUALSENSORNETWEB_DB_NAME=${POSTGRESQL_VIRTUALSENSORNET_WEB_DB_NAME}
}

function run_docker_container_postgresql() {
  echo "Running PostgreSQL Docker Container..."
  docker run -d --name $DOCKER_IMAGE_NAME_POSTGRESQL -p $POSTGRESQL_PORT:$POSTGRESQL_PORT $DOCKER_IMAGE_NAME_POSTGRESQL
   
}

function build_docker_image_rabbitmq() {
  echo "Building RabbitMQ Docker Image..."
  docker build -t $DOCKER_IMAGE_NAME_RABBITMQ rabbitmq
}

function run_docker_container_rabbitmq() {
  echo "Running RabbitMQ Docker Container..."
  docker run --name $DOCKER_IMAGE_NAME_RABBITMQ -d -p $RABBITMQ_PORT:$RABBITMQ_PORT -p $RABBITMQ_ADMIN_PORT:$RABBITMQ_ADMIN_PORT -v /opt/rabbitmq:/var/lib/rabbitmq $DOCKER_IMAGE_NAME_RABBITMQ
  #docker run --name osiris-rabbitmq_development -d -p 5672:5672 -p 15672:15672 -v /opt/rabbitmq:/var/lib/rabbitmq osiris-rabbitmq_development
}

function build_docker_image_sensornet_omcp() {
  #Generate the sensornet configuration file
  echo 'postgres.server.db='${POSTGRESQL_SENSORNET_DB_NAME} > sensornet-omcp/$OSIRIS_SENSORNET_CONFIG_FILE_NAME
