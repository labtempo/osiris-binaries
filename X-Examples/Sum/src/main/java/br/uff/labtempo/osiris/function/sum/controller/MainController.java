 ...
 */
package br.uff.labtempo.osiris.function.sum.controller;
...
public class MainController extends Controller {
    ...
    private final String REQUEST_PARAM = "input";
    private final String RESPONSE_PARAM = "output";
    ...
    public Response routing(Request request) throws MethodNotAllowedException, NotFoundException, InternalServerErrorException, NotImplementedException, BadRequestException {
        ...
        if (match(request.getResource(), Path.RESOURCE_FUNCTION_REQUEST.toString())) {
            ...
            switch (request.getMethod()) {
                case GET:
                    ResponseFnTo responseFnTo = execute(urlParams, request.getDate());
                    Response response = new ResponseBuilder().ok(responseFnTo, contentType).build();
                    return response;
                case POST:
                    RequestFnTo to = request.getContent(RequestFnTo.class);
    ...
    public synchronized ResponseFnTo execute(Map<String, String> params, Calendar calendar) throws NotFoundException, InternalServerErrorException, BadRequestException {
        List<Double> values = getValuesFromUriParams(params);
    ...
    private List<Double> getValuesFromUriParams(Map<String, String> params) throws BadRequestException {
        final String key = REQUEST_PARAM;
        try {
